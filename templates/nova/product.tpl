...
{if $pageName == 'product' && $inStock}
<div class="nova-sticky-buy" aria-hidden="false">
	<div class="container custom-container d-flex align-items-center gap-2">
		<div class="flex-grow-1 small fw-bold text-truncate">{$productName|escape}</div>
		<div class="fw-bold text-nowrap me-2" id="productStickyPrice">{Tools::displayPrice($price)}</div>
		<button type="button" class="nova-btn nova-btn--primary addtocart" data-id="{$product.id_product}">{'Add to Cart'|translate}</button>
	</div>
</div>
{/if}