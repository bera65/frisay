...
<div class="admin-panel p-3">
	<form method="post">
		...
		<div class="row g-3">
			...
			<div class="col-md-4">
				...
				<select name="discount_type" class="form-select">
					<option value="percent"{if $coupon.discount_type == 'percent'} selected{/if}>Yüzde (%)</option>
					<option value="fixed"{if $coupon.discount_type == 'fixed'} selected{/if}>Sabit Tutar (₺)</option>
				</select>
			</div>
			<div class="col-md-4">
			...
			<div class="col-md-4">
				<label class="form-label">Maks. Kullanım (0 = sınırsız)</label>
				<input type="number" name="max_uses" class="form-control" value="{$coupon.max_uses}" min="0">
			</div>
			<div class="col-md-4">
				<label class="form-label">Durum</label>
				<select name="active" class="form-select">
					<option value="1"{if $coupon.active} selected{/if}>Aktif</option>
					<option value="0"{if !$coupon.active} selected{/if}>Pasif</option>
				</select>
			</div>
			<div class="col-md-6">
		...
		<div class="d-flex gap-2 mt-4">
			...
			<a href="{$adminUrl}coupons" class="btn btn-outline-secondary">Geri</a>
		</div>
	</form>
</div>
