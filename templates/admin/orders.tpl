<div class="ps-order-filters mb-3">
	<a href="{$adminUrl}orders" class="ps-filter-pill{if $statusFilter == 0} active{/if}">{'Tümü'|adminT}</a>
...
<div class="ps-panel">
	<div class="ps-panel__head ps-panel__head--split">
		<h2>{'Siparişler'|adminT} {if $ordersTotal > 0}<span class="ps-panel__count">({$ordersTotal})</span>{/if}</h2>
	</div>
